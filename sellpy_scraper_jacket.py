...
try:
    ...
    while True:
        ...
        for article in soup.find_all("div", class_="item-tile-container"):
            ...
            dd_elements = article_soup.find_all('dd')
            p_elements = [x.find('p') for x in dd_elements]
            p_elements = [x for x in p_elements if x is not None]
            sizes = [int(x.get_text(strip=True)) for x in p_elements if x.get_text(strip=True).isnumeric()]
